...
object BillingManager {
    ...
    fun getCurrentEntitlements(): UserEntitlements =
        AccessControl.entitlementsFor(_userAccess.value.state)
    ...
    fun initialize(context: Context) {
        ...
        val trialEndsAt =
            if (prefs.contains("trial_ends_at")) prefs.getLong("trial_ends_at", 0) else null
        val premiumExpiresAt = if (prefs.contains("premium_expires_at")) prefs.getLong(
            "premium_expires_at",
            0
        ) else null
    ...
    fun activateTrial(context: Context) {
        val endsAt =
            System.currentTimeMillis() + (AppConfig.TRIAL_DURATION_DAYS * 24 * 60 * 60 * 1000L)
    ...
    private fun updateState(context: Context, newAccess: UserAccess) {
        ...
        context.getSharedPreferences("adshield_prefs", Context.MODE_PRIVATE).edit {
            ...
            if (newAccess.trialEndsAt != null) putLong(
                "trial_ends_at",
                newAccess.trialEndsAt
            ) else remove("trial_ends_at")
            if (newAccess.premiumExpiresAt != null) putLong(
                "premium_expires_at",
                newAccess.premiumExpiresAt
            ) else remove("premium_expires_at")
        }

        val isEffectivePremium =
            (newAccess.state == UserAccessState.PREMIUM || newAccess.state == UserAccessState.TRIAL)
    ...
    fun purchase(
        activity: Activity,
        @Suppress("UNUSED_PARAMETER") packageToBuy: Any?,
        onLoaders: (Boolean) -> Unit
    ) {
        onLoaders(true)
