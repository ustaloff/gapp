...
object FilterEngine {
    ...
    private val allowlist = setOf(
        ...
        "hdrezka.ac", "hdrezka.ag", "rezka.ag", "static.hdrezka.ac", "hls.hdrezka.ac",
    ...
    fun updateBlocklist(newRules: Set<String>) {
        ...
        for (rule in newRules) {
            val labels = rule.lowercase().split('.').reversed()
            var current = newRoot
            for (label in labels) {
        ...
        root = newRoot
        ruleCount = newCount
        android.util.Log.i("FilterEngine", "Trie updated. Rules: $newCount")
    }

    fun getRuleCount(): Int = ruleCount

    /**
     * High-performance Trie-based matching.
     * Time complexity: O(number_of_labels)
     * Space complexity: O(1) extra beyond the labels themselves
     */
    fun shouldBlock(domain: String?): Boolean {
        ...
        // 1. Safety Check: Allowlist (Check full domain and parents)
        ...
        // 2. Trie Matching (Check labels from end to start: com -> example -> ads)
        ...
        for (label in labels) {
            ...
            if (current.isEndOfRule) {
                // matched a rule (e.g. google.com matches ads.google.com)
